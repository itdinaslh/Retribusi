...
function loadTable() {
    ...
    $('#tblData').DataTable({
        ...
        orderMulti: false,
        ajax: {
            url: "/api/transport/tps/operator",
            type: "POST",
            dataType: "json"
        },
        columns: [
            ...
            {
                data: 'pegawaiId',
                render: function (data, type, row) { return "<button type='button' class='btn btn-sm btn-success mr-2 showMe' style='width:100%;' data-href='/transport/tps/operator/edit/?operatorId=" + row.pegawaiId + "'> Edit</button>" },
                sortable: false
            }
        ]
    });
...
function CheckOperator() {
    ...
    $.ajax({
        url: '/transport/tps/operator/check/?nik=' + nik,
        type: 'GET',        
        success: function (result) {
            if (result.failed) {
                exist = false;
...
function ChangeState() {
    ...
    if (exist == true) {
        $('#frmData').attr('action', '/transport/tps/operator/store');
        $('#btnSubmit').text("Submit");
        $('#theKota').prop('required', true);
    } else {
        $('#frmData').attr('action', '/transport/tps/operator/check');
        $('#btnSubmit').text("Cek Operator");
        ClearInput();
    }
}

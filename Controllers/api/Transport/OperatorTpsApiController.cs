...
[ApiController]
[Authorize]
public class OperatorTpsApiController : ControllerBase
{
    private readonly IPegawai repo;

    public OperatorTpsApiController(IPegawai repo) { this.repo = repo; }

    [HttpPost("/api/transport/tps/operator")]
    public async Task<IActionResult> DriverTable()
    {
        ...
        var init = repo.Pegawais.Where(x => x.RoleId == 4)
            .Select(x => new {
